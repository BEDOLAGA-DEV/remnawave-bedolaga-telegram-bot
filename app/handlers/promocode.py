            ...
            buttons.append(
                [
                    types.InlineKeyboardButton(
                        text=f'{name} ({days} дн.)',
                        callback_data=f'nz!_promo_sub:{sub["id"]}:{promo_code}',
                    )
                ]
            )
        buttons.append([types.InlineKeyboardButton(text='❌ Отмена', callback_data='nz!_back_to_menu')])
        await message.answer(
            texts.t(
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(handle_promo_subscription_select, F.data.startswith('nz!_promo_sub:'))
