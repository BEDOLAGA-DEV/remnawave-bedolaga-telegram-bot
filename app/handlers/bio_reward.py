...
async def _build_status_text(
    ...
    head = (
        ...
        '❗ <b>Важно знать про бесплатную подписку:</b>\n'
        '• Это <b>отдельная подписка</b>, она не связана с платными тарифами и не заменяет их\n'
        '• Для неё выделены <b>отдельные серверы</b> — их список, скорость и локации '
        'могут отличаться от платных\n'
        '• <b>Работоспособность не гарантируется</b>: серверы акции могут быть '
        'перегружены или временно недоступны, компенсации за простой не предусмотрены\n'
        '• Приоритетная поддержка и стабильные серверы доступны на платных тарифах\n\n'
