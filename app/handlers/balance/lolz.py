"""Handler for LOLZ balance top-up."""
...
async def _create_lolz_payment_and_respond(
    message_or_callback,
    db_user: User,
    db: AsyncSession,
    amount_kopeks: int,
    ...
    """
    Common logic for creating LOLZ payment and sending response.
    """
    ...
    result = await payment_service.create_lolz_payment(
        db=db,
        user_id=db_user.id,
    ...
    logger.info('LOLZ payment created', telegram_id=db_user.telegram_id, amount_rub=amount_rub)


@error_handler
async def process_lolz_payment_amount(
    ...
    # Validate amount
    min_amount = settings.LOLZ_MIN_AMOUNT_KOPEKS
    max_amount = settings.LOLZ_MAX_AMOUNT_KOPEKS
    ...
    await _create_lolz_payment_and_respond(
...
async def start_lolz_topup(
    ...
    """
    Start LOLZ top-up process - ask for amount.
    """
    texts = get_texts(db_user.language)
    ...
    await state.update_data(payment_method='lolz')

    min_amount = settings.LOLZ_MIN_AMOUNT_KOPEKS // 100
    max_amount = settings.LOLZ_MAX_AMOUNT_KOPEKS // 100
    display_name = settings.get_lolz_display_name()
