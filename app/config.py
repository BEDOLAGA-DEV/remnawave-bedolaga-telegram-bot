...
class Settings(BaseSettings):
    ...
    CABINET_TRUSTED_PROXIES: str = ""  # Comma-separated IPs/CIDRs of trusted reverse proxies (e.g. '127.0.0.1,10.0.0.0/8')
    ...
    # Example: https://your-proxy.workers.dev or http://localhost:8081
    ...
    def get_telegram_api_url(self) -> str | None:
        """Return custom Telegram API server URL or None (uses default api.telegram.org)."""
    ...
    def get_external_admin_bot_id(self) -> int | None:
        try:
            return (
                int(self.EXTERNAL_ADMIN_TOKEN_BOT_ID)
                if self.EXTERNAL_ADMIN_TOKEN_BOT_ID
                else None
            )
        except (
            TypeError,
            ValueError,
        ):  # pragma: no cover - защитная ветка для некорректных значений
            logger.warning(
