...
class AdminNotificationService:
        ...
        try:
            await self._record_subscription_event(
                ...
                extra={
                    ...
                    # Clamp to >=1 (upstream): start/end set with separate now()
                    # calls a few ms apart can floor a full day down to 0.
                    'trial_duration_days': (
            ...
            if subscription.start_date and subscription.end_date:
                ...
                # minus ~ms which floors to N-1. Clamp to >=1 (upstream) so a
                # sub-day delta never renders as "0 дней".
                actual_period_days = max(1, round(delta.total_seconds() / 86400))
    ...
    async def send_version_update_notification(self, current_version: str, latest_version, total_updates: int) -> bool:
        ...
        try:
            ...
            repo = getattr(settings, 'VERSION_CHECK_REPO', 'fr1ngg/remnawave-nozapret-telegram-bot')
