...
async def get_renewals_stats(
    ...
    try:
        ...
        renewals_by_date = {
            (row.date.isoformat() if hasattr(row.date, 'isoformat') else str(row.date)): row.count
            for row in daily_renewals_query
        }
        ...
        regular_by_date = {
            (row.date.isoformat() if hasattr(row.date, 'isoformat') else str(row.date)): row.count
            for row in daily_regular_query
        }
        ...
        while curr_date <= end_date_limit:
            ...
            renewals_count = renewals_by_date.get(curr_str, 0)
            trials_count = trials_by_date.get(curr_str, 0)
            regular_count = regular_by_date.get(curr_str, 0)
