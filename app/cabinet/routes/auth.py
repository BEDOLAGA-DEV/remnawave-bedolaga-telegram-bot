...
async def _create_or_login_user_from_oidc_claims(
    ...
    *,
    ...
    # Order: explicit referral_code (passed in from the request by the
    # caller), then Redis pending_referral written by /start ref_XYZ.
    # The Redis fallback was previously missing from this OIDC endpoint —
    # users who hit /start ref_XYZ then logged in via the cabinet's OIDC
    # flow lost attribution.
    # NOTE: this helper receives ``referral_code`` as a parameter, so we use
    # it directly rather than ``request.referral_code`` (no ``request`` in
    # scope here).
    referrer_id = None
    if not user:
        if referral_code:
    ...
    # Redis pending_referral is the only TRUSTED source for retroactive
    # attach (the inbound referral_code is client-controlled — see security
    # comment in /telegram above).
    if not is_new_user:
        from app.services.referral_service import attach_referrer_if_missing
