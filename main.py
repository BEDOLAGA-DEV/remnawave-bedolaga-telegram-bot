...
async def main():
    ...
    try:
        ...
        async with timeline.stage(
            ...
            # Переключатели уведомлений истёкшим раньше жили в data/notification_settings.json —
            # один раз переносятся в базу, чтобы прежние «Выкл» операторов не пропали.
            try:
                from app.database.database import AsyncSessionLocal
                ...
                if imported:
                    stage.log(f'Настройки уведомлений перенесены из файла в базу: {len(imported)}')
            except Exception as error:
                logger.error('❌ Не удалось перенести настройки уведомлений из файла', error=error)
