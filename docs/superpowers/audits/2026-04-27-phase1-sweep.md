...
| 68 | P11 | migrations/alembic/versions/ | - | `ADD COLUMN NOT NULL` audit: every `op.add_column(..., nullable=False, ...)` in upgrade paths is paired with `server_default=...`. `alter_column` to NOT NULL is gated by explicit backfill (0051) or appears only in `downgrade()` with `# WARNING` comments (0019, 0022, 0023, 0039). Pattern clean | info | accept-with-rationale | accept |
| 69 | P11 | migrations/alembic/versions/ | - | `DROP COLUMN`/`DROP TABLE` audit: all hits live in `downgrade()` functions paired with their `upgrade()` create. No upgrade-side data loss. Already-shipped, immutable per Alembic discipline | info | accept-with-rationale | accept |
| 70 | P11 | migrations/alembic/versions/0003_add_partner_system.py | 64 | `partner_applications` FKs `user_id` (line 64), `processed_by` (line 70) — no `index=True` and no later `op.create_index(...)`. FK lookups full-scan. Already shipped — immutable | medium | real-bug | queue-phase2 |
| 71 | P11 | migrations/alembic/versions/0011_add_rbac_tables.py | 55 | `user_roles.user_id` lacks dedicated index (UniqueConstraint(user_id, role_id) helps user_id-prefix lookups). `role_id` (line 56) has neither dedicated index nor index-prefix coverage; `role_id`-only queries full-scan | medium | real-bug | queue-phase2 |
| 72 | P11 | migrations/alembic/versions/0011_add_rbac_tables.py | 69 | `access_policies.role_id` (line 69), `access_policies.created_by` (line 76) — no `index=True`, no `create_index`. Cascade-delete on role triggers full scan | medium | real-bug | queue-phase2 |
| 73 | P11 | migrations/alembic/versions/0067_add_user_reviews.py | 31 | `user_reviews.user_id` FK (`ondelete=CASCADE`) has no index. Cascade-delete on user triggers full scan; "reviews by user" lookups also slow | medium | real-bug | queue-phase2 |
