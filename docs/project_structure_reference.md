...
- `app/cabinet/routes/subscription_modules/helpers.py` — Python-модуль
  Классы: нет
  Функции: `resolve_subscription` — Resolve target subscription: by ID in multi-tariff mode, or legacy fallback., `build_premium_traffic_info` — Остатки по премиум-сквадам подписки для карточки расхода., `ensure_subscription_has_tariff` — Докупки старой подписке не продаются — сперва переход на тариф.
...
- `app/services/premium_traffic_service.py` — Python-модуль
  Классы: `PremiumTrafficService` (31 методов)
...
- `migrations/alembic/versions/0127_user_reminders.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0128_create_subscription_premium_traffic.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
...
- `tests/services/test_premium_traffic_service.py` — Python-модуль
  Классы: `FakeRemnawaveApi` (4 методов), `TestUsageCollection` (6 методов), `TestDecisions` (13 методов), `TestNewStatePeriod` (6 методов), `TestOrphanedLimits` (10 методов), `TestPanelUserCache` (4 методов), `TestFirstDayCorrection` (5 методов), `TestIntervalSettings` (1 методов), `TestNotifications` (6 методов)
  Функции: нет
...
- `tests/utils/test_premium_traffic_period.py` — Python-модуль
  Классы: `TestNormalizeMode` (3 методов), `TestCalendarModes` (4 методов), `TestCalendarModesFollowTheBotTimezone` (4 методов), `TestRollingMonth` (6 методов), `TestNoReset` (2 методов), `TestPanelResetCorrection` (7 методов), `TestAnchor` (3 методов), `TestNaiveDatetimes` (1 методов)
  Функции: нет
