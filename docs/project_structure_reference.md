...
- `app/external/remnawave_api.py` — Python-модуль
  ...
  Функции: `coerce_panel_user_id` — Привести локально хранимый идентификатор к числовому id панели., `is_expire_in_past_error` — Панель отвергла ``expireAt`` как прошедшую дату., `is_user_not_found_error` — Панель не нашла пользователя (удалён/протух идентификатор)., `format_bytes`, `parse_bytes`, `test_api_connection`
