...
- `tests/services/test_user_presence_activity.py` — Python-модуль
  Классы: нет
  Функции: `presence`, `test_stale_user_is_bumped_and_visit_is_logged`, `test_fresh_user_is_left_alone`, `test_short_pause_bumps_without_a_visit` — Между 5 минутами и получасом метка двигается, но визит не пишется — сессия та же., `test_never_seen_user_gets_a_visit`, `test_naive_timestamp_is_treated_as_utc` — Старые строки без таймзоны не должны ронять авторизацию сравнением naive/aware., `test_visit_is_gated_by_the_flag`, `test_visit_row_shape`, `test_cabinet_dependency_touches_last_activity`, `test_miniapp_auth_touches_last_activity_and_writes_visit`, `test_timeline_marks_visits`, `test_bot_click_is_attributed_by_telegram_id`, `test_middleware_passes_telegram_id_explicitly` — Пин: middleware зовёт запись с telegram_id, а не с двусмысленным user_id., `test_remember_task_holds_until_done`, `test_remember_task_ignores_non_tasks` — Тесты middleware подменяют create_task заглушкой, возвращающей None.
