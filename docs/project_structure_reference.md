...
- `app/cabinet/routes/activity.py` — Python-модуль
  Классы: `ScreenViewRequest`
  Функции: `report_screen_view` — Записать открытие экрана кабинета (fire-and-forget, ответ не ждёт записи).
...
- `app/middlewares/button_stats.py` — Python-модуль
  Классы: `ButtonStatsMiddleware` (7 методов)
  Функции: нет
