...
- `app/services/reachability/geo_catalog.py` — Python-модуль
  ...
  Функции: `catalog_params` — Query к сервису: пустые фильтры не уходят, округ — латиницей, потолок городов — в рамках 1..5000.
...
- `app/services/reachability/geo_result.py` — Python-модуль
  Классы: нет
  Функции: `is_result_verdict`, `normalize_rows` — Строки сервиса → строки кабинета; не-словарь в списке пропускается, а не роняет задачу., `geo_summary`, `scope_label` — Заголовок для истории: «сайты · проводной · округ ЦФО · МТС · до 30 городов».
...
- `app/services/reachability/service.py` — Python-модуль
  Классы: `ReachabilityDisabled` (1 методов), `ReachabilityUnhealthy` (1 методов), `ReachabilityBusy` (1 методов), `PanelUnavailable` (1 методов), `JobNotFound`, `Health` (1 методов), `Quote`, `ParsedConfig`, `ParsedInput`, `ReachabilityService` (55 методов)
  Функции: нет
