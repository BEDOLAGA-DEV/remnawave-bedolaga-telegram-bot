...
async def _clean_wl_orphans(api, svc, sub, user, keeper_main_uuid, *, apply: bool, delete: bool) -> int:
    """Disable/delete stale _wl accounts that are NOT the keeper main's _wl.

    The ONLY correct _wl is '<keeper_main_username>_wl'. Older naming conventions
    can leave extra _wl accounts (e.g. 'u_<tg>_<sub_id>_wl', 'u_<tg>_<short>_wl',
    'u_<tg>_wl') alive on the panel. This finds them by name across both prefixes
    and id variants and removes them — but it excludes the keeper main and the
    keeper _wl BY UUID, so it can never touch the good accounts. Returns the
    number of orphans found (acted on when apply=True).
    """
    if not keeper_main_uuid or not user or not user.telegram_id:
        return 0
    keeper_main = await api.get_user_by_uuid(keeper_main_uuid)
    keeper_main_name = getattr(keeper_main, 'username', None)
    if not keeper_main_name:
        return 0

    keeper_wl_name = svc._derive_wl_username(keeper_main_name, None, None)
    keeper_wl = await api.get_user_by_username(keeper_wl_name)
    keeper_uuids = {keeper_main_uuid}
    if keeper_wl and getattr(keeper_wl, 'uuid', None):
        keeper_uuids.add(keeper_wl.uuid)

    tg = user.telegram_id
    bases = {f'u_{tg}', f'user_{tg}'}
    sub_id = getattr(sub, 'id', None)
    if sub_id:
        bases |= {f'u_{tg}_{sub_id}', f'user_{tg}_{sub_id}'}
    short_id = getattr(sub, 'remnawave_short_id', None)
    if short_id:
        bases |= {f'u_{tg}_{short_id}', f'user_{tg}_{short_id}'}
    cand_names = {f'{b}_wl' for b in bases}
    cand_names.discard(keeper_wl_name)  # never target the keeper by name
    ...
    for name in sorted(cand_names):
        ...
        if not acc_uuid or acc_uuid in keeper_uuids or acc_uuid in seen:
            continue
        seen.add(acc_uuid)
        count += 1
        print(f'      orphan _wl: {name!r} uuid={acc_uuid}' + ('' if apply else ' (dry-run)'))
        if not apply:
            continue
        try:
            if delete:
                await api.delete_user(acc_uuid)
            else:
                await api.disable_user(acc_uuid)
            print(f'        -> {"deleted" if delete else "disabled"}.')
        except Exception as e:
            msg = str(e).lower()
            if not ('already' in msg or 'not found' in msg or 'not exist' in msg or '404' in msg):
                print(f'        WARN: could not act on {name}: {e}')
    return count
...
async def main() -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        async with svc.get_api_client() as api:
            ...
            for sub in subs:
                ...
                main_correct = bool(existing) and any(acc.uuid == sub.remnawave_uuid for _, acc in existing)

                # Main is correctly bound (to a legacy account), or there is no
                # legacy account at all (the bound per-subscription main IS the
                # keeper): in both cases hunt stale _wl orphans against it.
                if not existing or main_correct:
                    n = await _clean_wl_orphans(
                        api, svc, sub, user, sub.remnawave_uuid,
                        apply=args.apply, delete=args.delete_duplicates,
                    )
                    if n:
                        wl_orphans += n
                        print(f'  sub {sub.id} (tg {user.telegram_id}): {n} orphan _wl account(s).')
                    continue
                ...
                if args.apply:
                    ...
                    # Neutralise the duplicate main + its _wl (recoverable disable by default).
                ...
                # Hunt stale _wl orphans against the legacy keeper (dry-run reports
                # what apply would remove; the keeper main + its _wl are uuid-excluded).
                n = await _clean_wl_orphans(
                    api, svc, sub, user, legacy_user.uuid,
                    apply=args.apply, delete=args.delete_duplicates,
                )
                if n:
                    wl_orphans += n
    ...
    print(
        ...
        f'  orphan _wl accounts:  {wl_orphans}'
    )
    if not args.apply and (victims or wl_orphans):
        print(f'\nDry-run only. Re-run with --apply to repoint victims and {dup_action.lower()} orphans.')
    return 0
